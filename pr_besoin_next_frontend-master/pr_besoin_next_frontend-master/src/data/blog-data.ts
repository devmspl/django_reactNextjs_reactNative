import { IBlogDataType } from '@/types/blog-type';
import blog_1 from '@/assets/images/blog/blog_img_01.png';
import blog_2 from '@/assets/images/blog/blog_img_02.png';
import blog_3 from '@/assets/images/blog/blog_img_03.png';
import blog_4 from '@/assets/images/blog/blog_img_04.jpg';
import blog_5 from '@/assets/images/blog/blog_img_05.jpg';
import blog_6 from '@/assets/images/blog/blog_img_06.jpg';
import blog_7 from '@/assets/images/blog/blog_img_07.jpg';
import blog_8 from '@/assets/images/blog/blog_img_08.jpg';
import blog_9 from '@/assets/images/blog/blog_img_09.jpg';
import blog_10 from '@/assets/images/blog/blog_img_10.jpg';
import blog_11 from '@/assets/images/blog/blog_img_11.jpg';
import blog_12 from '@/assets/images/blog/blog_img_12.jpg';
import blog_13 from '@/assets/images/blog/blog_img_13.jpg';
import blog_14 from '@/assets/images/blog/blog_img_14.jpg';
import blog_15 from '@/assets/images/blog/blog_img_15.jpg';
import blog_16 from '@/assets/images/blog/blog_img_16.jpg';
import blog_17 from '@/assets/images/blog/blog_img_17.jpg';
import blog_grid_1 from '@/assets/images/blog/blog_img_18.jpg';
import blog_grid_2 from '@/assets/images/blog/blog_img_19.jpg';
import blog_grid_3 from '@/assets/images/blog/blog_img_20.jpg';
import blog_grid_4 from '@/assets/images/blog/blog_img_21.jpg';
import blog_grid_5 from '@/assets/images/blog/blog_img_22.jpg';
import blog_grid_6 from '@/assets/images/blog/blog_img_23.jpg';
import blog_full_1 from '@/assets/images/blog/blog_img_24.jpg';
import blog_full_2 from '@/assets/images/blog/blog_img_25.jpg';
import blog_full_3 from '@/assets/images/blog/blog_img_26.jpg';
import blog_full_4 from '@/assets/images/blog/blog_img_27.jpg';
import blog_full_5 from '@/assets/images/blog/blog_img_28.jpg';
import blog_full_6 from '@/assets/images/blog/blog_img_29.jpg';
import blog_full_7 from '@/assets/images/blog/blog_img_30.jpg';
import blog_full_8 from '@/assets/images/blog/blog_img_31.jpg';

const blog_data:IBlogDataType[] = [
  // blog one
  {
    id:1,
    img:blog_1,
    tags:['Developer','Code'],
    title:'Print, publishing qui visual layout mockups.',
    author:'James Brower',
    date:'23 APR, 2023',
    desc:'Tomfoolery crikey bits and bobs brilliant bamboozled down the pub amongst brolly hanky panky, cack bonnet arse over tit burke bugger all mate bodge. cillum dolore eu fugiat pariatur. Excepteur sint occaecat cupidatat non proident, sunt in culpa qui officia deserunt mollit anim id est laborum.Suspendisse interdum consectetur libero id faucibu nisl. Lacus vel facilisis volutpat est velit egestas.Tempus imperdiet nulla malesuada pellentesque elit eget gravida cum. Sit amet ris nullam eget felis. Enim praesent elementum facilisis leo. Ultricies leo integer.',
    blog:'blog-one'
  },
  {
    id:2,
    img:blog_2,
    tags:['Design','Art'],
    title:'Designer’s checklist for every UX/UI project.',
    author:'Julie Margot',
    date:'25 APR, 2023',
    desc:'Tomfoolery crikey bits and bobs brilliant bamboozled down the pub amongst brolly hanky panky, cack bonnet arse over tit burke bugger all mate bodge. cillum dolore eu fugiat pariatur. Excepteur sint occaecat cupidatat non proident, sunt in culpa qui officia deserunt mollit anim id est laborum.Suspendisse interdum consectetur libero id faucibu nisl. Lacus vel facilisis volutpat est velit egestas.Tempus imperdiet nulla malesuada pellentesque elit eget gravida cum. Sit amet ris nullam eget felis. Enim praesent elementum facilisis leo. Ultricies leo integer.',
    blog:'blog-one'
  },
  {
    id:3,
    img:blog_3,
    tags:['Solution','Work'],
    title:'Make more productive work flow in few steps.',
    author:'Martin Cooley',
    date:'26 APR, 2023',
    desc:'Tomfoolery crikey bits and bobs brilliant bamboozled down the pub amongst brolly hanky panky, cack bonnet arse over tit burke bugger all mate bodge. cillum dolore eu fugiat pariatur. Excepteur sint occaecat cupidatat non proident, sunt in culpa qui officia deserunt mollit anim id est laborum.Suspendisse interdum consectetur libero id faucibu nisl. Lacus vel facilisis volutpat est velit egestas.Tempus imperdiet nulla malesuada pellentesque elit eget gravida cum. Sit amet ris nullam eget felis. Enim praesent elementum facilisis leo. Ultricies leo integer.',
    blog:'blog-one'
  },
  // blog two
  {
    id:4,
    img:blog_4,
    tags:['Design','Work'],
    title:'Print, publishing qui visual layout mockups.',
    author:'Martin Cooley',
    date:'27 APR, 2023',
    desc:'This response is important for our ability to learn from mistakes, but words',
    blog:'blog-two'
  },
  {
    id:5,
    img:blog_5,
    tags:['Fashion','Work'],
    title:'Designer’s checklist for every UX/UI project.',
    author:'Martin Cooley',
    date:'28 APR, 2023',
    desc:'This response is important for our ability to learn from mistakes, but words',
    blog:'blog-two'
  },
  {
    id:6,
    img:blog_6,
    tags:['Art','Work'],
    title:'Make more productive work flow in few steps.',
    author:'Martin Cooley',
    date:'30 APR, 2023',
    desc:'This response is important for our ability to learn from mistakes, but words',
    blog:'blog-two'
  },
  // blog three
  {
    id:7,
    img:blog_7,
    tags:['Solution'],
    title:'Print, publishing qui visual ux quis layout mockups.',
    author:'Martin Cooley',
    date:'30 MAY, 2023',
    featured:true,
    desc:'Tomfoolery crikey bits and bobs brilliant bamboozled down the pub amongst brolly hanky panky, cack bonnet arse over tit burke bugger all mate bodge. cillum dolore eu fugiat pariatur. Excepteur sint occaecat cupidatat non proident, sunt in culpa qui officia deserunt mollit anim id est laborum.Suspendisse interdum consectetur libero id faucibu nisl. Lacus vel facilisis volutpat est velit egestas.Tempus imperdiet nulla malesuada pellentesque elit eget gravida cum. Sit amet ris nullam eget felis. Enim praesent elementum facilisis leo. Ultricies leo integer.',
    blog:'blog-three'
  },
  {
    id:8,
    img:blog_8,
    tags:['Design'],
    title:'Designer’s Checklist for Every UX/UI Project.',
    author:'John Smith',
    date:'1 MAY, 2023',
    desc:'Tomfoolery crikey bits and bobs brilliant bamboozled down the pub amongst brolly hanky panky, cack bonnet arse over tit burke bugger all mate bodge. cillum dolore eu fugiat pariatur. Excepteur sint occaecat cupidatat non proident, sunt in culpa qui officia deserunt mollit anim id est laborum.Suspendisse interdum consectetur libero id faucibu nisl. Lacus vel facilisis volutpat est velit egestas.Tempus imperdiet nulla malesuada pellentesque elit eget gravida cum. Sit amet ris nullam eget felis. Enim praesent elementum facilisis leo. Ultricies leo integer.',
    blog:'blog-three'
  },
  // blog four
  {
    id:9,
    img:blog_9,
    tags:['Solution'],
    title:'Print, publishing qui visual ux quis layout mockups.',
    author:'John Smith',
    date:'2 MAY, 2023',
    featured:true,
    desc:'Tomfoolery crikey bits and bobs brilliant bamboozled down the pub amongst brolly hanky panky, cack bonnet arse over tit burke bugger all mate bodge. cillum dolore eu fugiat pariatur. Excepteur sint occaecat cupidatat non proident, sunt in culpa qui officia deserunt mollit anim id est laborum.Suspendisse interdum consectetur libero id faucibu nisl. Lacus vel facilisis volutpat est velit egestas.Tempus imperdiet nulla malesuada pellentesque elit eget gravida cum. Sit amet ris nullam eget felis. Enim praesent elementum facilisis leo. Ultricies leo integer.',
    blog:'blog-four'
  },
  {
    id:10,
    img:blog_10,
    tags:['Design'],
    title:'Designer’s Checklist for Every UX/UI Project.',
    author:'Martin Cooley',
    date:'5 MAY, 2023',
    desc:'Tomfoolery crikey bits and bobs brilliant bamboozled down the pub amongst brolly hanky panky, cack bonnet arse over tit burke bugger all mate bodge. cillum dolore eu fugiat pariatur. Excepteur sint occaecat cupidatat non proident, sunt in culpa qui officia deserunt mollit anim id est laborum.Suspendisse interdum consectetur libero id faucibu nisl. Lacus vel facilisis volutpat est velit egestas.Tempus imperdiet nulla malesuada pellentesque elit eget gravida cum. Sit amet ris nullam eget felis. Enim praesent elementum facilisis leo. Ultricies leo integer.',
    blog:'blog-four'
  },
  // blog postbox
  {
    id:11,
    img:blog_11,
    grid_img:blog_grid_1,
    img_full:blog_full_1,
    tags:['Solution'],
    title:'Print, publishing qui visual ux quis layout mockups.',
    author:'Martin Cooley',
    date:'12 MAY, 2023',
    featured:true,
    desc:'Tomfoolery crikey bits and bobs brilliant bamboozled down the pub amongst brolly hanky panky, cack bonnet arse over tit burke bugger all mate bodge. cillum dolore eu fugiat pariatur. Excepteur sint occaecat cupidatat non proident, sunt in culpa qui officia deserunt mollit anim id est laborum.Suspendisse interdum consectetur libero id faucibu nisl. Lacus vel facilisis volutpat est velit egestas.Tempus imperdiet nulla malesuada pellentesque elit eget gravida cum. Sit amet ris nullam eget felis. Enim praesent elementum facilisis leo. Ultricies leo integer.',
    blog:'blog-postbox'
  },
  {
    id:12,
    img:blog_12,
    grid_img:blog_grid_2,
    img_full:blog_full_2,
    tags:['Business'],
    title:'Medieval origins to the digital era,  everything there is to know.',
    author:'John Smith',
    date:'10 April, 2023',
    featured:true,
    desc:'Tomfoolery crikey bits and bobs brilliant bamboozled down the pub amongst brolly hanky panky, cack bonnet arse over tit burke bugger all mate bodge. cillum dolore eu fugiat pariatur. Excepteur sint occaecat cupidatat non proident, sunt in culpa qui officia deserunt mollit anim id est laborum.Suspendisse interdum consectetur libero id faucibu nisl. Lacus vel facilisis volutpat est velit egestas.Tempus imperdiet nulla malesuada pellentesque elit eget gravida cum. Sit amet ris nullam eget felis. Enim praesent elementum facilisis leo. Ultricies leo integer.',
    blog:'blog-postbox'
  },
  {
    id:13,
    img:blog_13,
    grid_img:blog_grid_3,
    img_full:blog_full_3,
    tags:['Marketing'],
    title:'Quis nostrud exercitation ullamco laboris nisi ut aliquip commodo cons.',
    author:'James J. Allen',
    date:'5 May, 2023',
    featured:true,
    desc:'Tomfoolery crikey bits and bobs brilliant bamboozled down the pub amongst brolly hanky panky, cack bonnet arse over tit burke bugger all mate bodge. cillum dolore eu fugiat pariatur. Excepteur sint occaecat cupidatat non proident, sunt in culpa qui officia deserunt mollit anim id est laborum.Suspendisse interdum consectetur libero id faucibu nisl. Lacus vel facilisis volutpat est velit egestas.Tempus imperdiet nulla malesuada pellentesque elit eget gravida cum. Sit amet ris nullam eget felis. Enim praesent elementum facilisis leo. Ultricies leo integer.',
    blog:'blog-postbox'
  },
  {
    id:14,
    img:blog_14,
    grid_img:blog_grid_4,
    img_full:blog_full_4,
    tags:['Developer'],
    title:'Designer’s Checklist for Every UX/UI Project.',
    author:'Paul R. Bruns',
    date:'15 March, 2023',
    featured:true,
    desc:'Tomfoolery crikey bits and bobs brilliant bamboozled down the pub amongst brolly hanky panky, cack bonnet arse over tit burke bugger all mate bodge. cillum dolore eu fugiat pariatur. Excepteur sint occaecat cupidatat non proident, sunt in culpa qui officia deserunt mollit anim id est laborum.Suspendisse interdum consectetur libero id faucibu nisl. Lacus vel facilisis volutpat est velit egestas.Tempus imperdiet nulla malesuada pellentesque elit eget gravida cum. Sit amet ris nullam eget felis. Enim praesent elementum facilisis leo. Ultricies leo integer.',
    blog:'blog-postbox'
  },
  {
    id:15,
    img:blog_15,
    grid_img:blog_grid_5,
    img_full:blog_full_5,
    tags:['Business'],
    title:'10 days quick challange forboost visitors.',
    author:'Stacey J. Meikle',
    date:'3 Feb, 2023',
    featured:true,
    desc:'Tomfoolery crikey bits and bobs brilliant bamboozled down the pub amongst brolly hanky panky, cack bonnet arse over tit burke bugger all mate bodge. cillum dolore eu fugiat pariatur. Excepteur sint occaecat cupidatat non proident, sunt in culpa qui officia deserunt mollit anim id est laborum.Suspendisse interdum consectetur libero id faucibu nisl. Lacus vel facilisis volutpat est velit egestas.Tempus imperdiet nulla malesuada pellentesque elit eget gravida cum. Sit amet ris nullam eget felis. Enim praesent elementum facilisis leo. Ultricies leo integer.',
    blog:'blog-postbox'
  },
  {
    id:16,
    img:blog_16,
    grid_img:blog_grid_6,
    img_full:blog_full_6,
    tags:['Architecture'],
    title:'Easy way to boost your business.',
    author:'Shawn E. Palmer',
    date:'5 Jan, 2023',
    featured:true,
    desc:'Tomfoolery crikey bits and bobs brilliant bamboozled down the pub amongst brolly hanky panky, cack bonnet arse over tit burke bugger all mate bodge. cillum dolore eu fugiat pariatur. Excepteur sint occaecat cupidatat non proident, sunt in culpa qui officia deserunt mollit anim id est laborum.Suspendisse interdum consectetur libero id faucibu nisl. Lacus vel facilisis volutpat est velit egestas.Tempus imperdiet nulla malesuada pellentesque elit eget gravida cum. Sit amet ris nullam eget felis. Enim praesent elementum facilisis leo. Ultricies leo integer.',
    blog:'blog-postbox'
  },
  {
    id:17,
    img:blog_17,
    img_full:blog_full_7,
    tags:['Development'],
    title:'Introducing new tools for your design.',
    author:'Corrie H. Cates',
    date:'20 June, 2023',
    featured:true,
    desc:'Tomfoolery crikey bits and bobs brilliant bamboozled down the pub amongst brolly hanky panky, cack bonnet arse over tit burke bugger all mate bodge. cillum dolore eu fugiat pariatur. Excepteur sint occaecat cupidatat non proident, sunt in culpa qui officia deserunt mollit anim id est laborum.Suspendisse interdum consectetur libero id faucibu nisl. Lacus vel facilisis volutpat est velit egestas.Tempus imperdiet nulla malesuada pellentesque elit eget gravida cum. Sit amet ris nullam eget felis. Enim praesent elementum facilisis leo. Ultricies leo integer.',
    blog:'blog-postbox'
  },
  {
    id:18,
    img:blog_15,
    img_full:blog_full_8,
    tags:['Development'],
    title:'Speaking remotely at WordCamp US.',
    author:'Corrie H. Cates',
    date:'25 June, 2023',
    featured:true,
    desc:'Tomfoolery crikey bits and bobs brilliant bamboozled down the pub amongst brolly hanky panky, cack bonnet arse over tit burke bugger all mate bodge. cillum dolore eu fugiat pariatur. Excepteur sint occaecat cupidatat non proident, sunt in culpa qui officia deserunt mollit anim id est laborum.Suspendisse interdum consectetur libero id faucibu nisl. Lacus vel facilisis volutpat est velit egestas.Tempus imperdiet nulla malesuada pellentesque elit eget gravida cum. Sit amet ris nullam eget felis. Enim praesent elementum facilisis leo. Ultricies leo integer.',
    blog:'blog-postbox'
  },
]

export default blog_data;